var IpolTVDeconvolutionItem,
  __hasProp = Object.prototype.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor; child.__super__ = parent.prototype; return child; };

IpolTVDeconvolutionItem = (function(_super) {

  __extends(IpolTVDeconvolutionItem, _super);

  function IpolTVDeconvolutionItem(name) {
    var disk, gaussian;
    if (name == null) name = "IpolTVDeconvolution";
    IpolTVDeconvolutionItem.__super__.constructor.call(this);
    this.add_attr({
      lambda: new ConstrainedVal(20, {
        min: 0.01
      }),
      kernel: new Choice
    });
    disk = new DiskKernel;
    gaussian = new GaussianKernel;
    this.kernel.lst.push(disk);
    this.kernel.lst.push(gaussian);
    this._name.set(name);
    this._viewable.set(false);
    this._computation_mode.set(false);
    this.add_child(new ImgSetItem('input'));
    this.add_output(new ImgSetItem('output'));
  }

  IpolTVDeconvolutionItem.prototype.get_model_editor_parameters = function(res) {
    return res.model_editor["kernel"] = ModelEditorItem_ChoiceWithEditableItems;
  };

  IpolTVDeconvolutionItem.prototype.accept_child = function(ch) {
    return ch instanceof ImgItem || ch instanceof FileItem;
  };

  IpolTVDeconvolutionItem.prototype.is_app_data = function(item) {
    if (item instanceof TreeAppData) {
      return true;
    } else {
      return false;
    }
  };

  IpolTVDeconvolutionItem.prototype.get_app_data = function() {
    var it;
    it = this.get_parents_that_check(this.is_app_data);
    return it[0];
  };

  IpolTVDeconvolutionItem.prototype.do_it = function() {
    var app_data, child, _i, _len, _ref;
    app_data = this.get_app_data();
    _ref = this._output[0]._children;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      child = _ref[_i];
      this._output[0].rem_child(child);
      app_data.delete_from_tree(child);
    }
    return TreeItem_Computable._do_it_rec(this);
  };

  IpolTVDeconvolutionItem.prototype.z_index = function() {};

  return IpolTVDeconvolutionItem;

})(TreeItem_Computable);
var ImgSetItem,
  __hasProp = Object.prototype.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor; child.__super__ = parent.prototype; return child; };

ImgSetItem = (function(_super) {

  __extends(ImgSetItem, _super);

  function ImgSetItem(name) {
    if (name == null) name = 'ImgSetItem';
    ImgSetItem.__super__.constructor.call(this);
    this._name.set(name);
  }

  ImgSetItem.prototype.accept_child = function(ch) {
    return ch instanceof ImgItem || ch instanceof FileItem;
  };

  return ImgSetItem;

})(TreeItem);
var TreeAppApplication_IpolTVDeconvolution,
  __hasProp = Object.prototype.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor; child.__super__ = parent.prototype; return child; };

TreeAppApplication_IpolTVDeconvolution = (function(_super) {

  __extends(TreeAppApplication_IpolTVDeconvolution, _super);

  function TreeAppApplication_IpolTVDeconvolution() {
    var _ina,
      _this = this;
    TreeAppApplication_IpolTVDeconvolution.__super__.constructor.call(this);
    this.name = 'Total Variation Deconvolution';
    this.powered_with = 'Ipol';
    this.publication_link = 'http://www.ipol.im/pub/art/2012/g-tvdc/';
    _ina = function(app) {
      var _ref, _ref2;
      return app.data.focus.get() !== ((_ref = app.selected_canvas_inst()) != null ? (_ref2 = _ref[0]) != null ? _ref2.cm.view_id : void 0 : void 0);
    };
    this.actions.push({
      ico: "data:image/png;base64,",
      siz: 1,
      txt: "Ipol Total Variation Deconvolution",
      ina: _ina,
      fun: function(evt, app) {
        var ipolTVDeconvolution;
        app.undo_manager.snapshot();
        return ipolTVDeconvolution = _this.add_item_depending_selected_tree(app.data, IpolTVDeconvolutionItem);
      }
    });
  }

  return TreeAppApplication_IpolTVDeconvolution;

})(TreeAppApplication);
var GaussianKernel,
  __hasProp = Object.prototype.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor; child.__super__ = parent.prototype; return child; };

GaussianKernel = (function(_super) {

  __extends(GaussianKernel, _super);

  function GaussianKernel() {
    GaussianKernel.__super__.constructor.call(this);
    this.add_attr({
      _name: "gaussian",
      _sigma: 1.0
    });
    this.add_attr({
      sigma: this._sigma
    });
  }

  GaussianKernel.prototype.toString = function() {
    return this._name.get();
  };

  return GaussianKernel;

})(Model);
var DiskKernel,
  __hasProp = Object.prototype.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor; child.__super__ = parent.prototype; return child; };

DiskKernel = (function(_super) {

  __extends(DiskKernel, _super);

  function DiskKernel() {
    DiskKernel.__super__.constructor.call(this);
    this.add_attr({
      _name: "disk",
      _radius: 1.0
    });
    this.add_attr({
      radius: this._radius
    });
  }

  DiskKernel.prototype.toString = function() {
    return this._name.get();
  };

  return DiskKernel;

})(Model);
